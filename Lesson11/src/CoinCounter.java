...
public class CoinCounter {
    ...
    int recursiveNumberOfCoinsRequired(int totalSum) {
        ...
        TODO: Create a recursive numberOfCoinsRequired method, making the unit tests pass.
         ...
         - What are your base cases?           when totalSum = 0, number of coins is 0
         - What is your recursive step?   number of Coins = fn(totalSum) + fn(totalSum - biggest denomination that is less than it)
         */
        if (totalSum == 0) {
            return 0;
        }
        else if (inDenominations(totalSum) ) {
            return 1;
        }
        else if (totalSum < 0 || lessThanSmallest(totalSum) ) {
...
<<<<<<< HEAD
        ...
        for (int d : denominations){
            int test = recursiveNumberOfCoinsRequired(totalSum - d);
            if (test < result_tester)  {
                best_denom = d;
                result_tester = test;
            }
        }
        ...
        return  1 + recursiveNumberOfCoinsRequired(totalSum - best_denom);
    }
    public boolean lessThanSmallest(int n ){
        return (n < denominations[0]) && n > 0;
    }
    public boolean inDenominations(int n ){
        for (int d : denominations){
            if (d == n) {
                return true;
            }
